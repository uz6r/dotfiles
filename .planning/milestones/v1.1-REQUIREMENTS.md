# Requirements Archive: v1.1 Shell Optimization
...
- [x] **PROF-01**: zsh startup timing script measures load time of .zshrc
- [x] **PROF-02**: Profiler identifies which plugins/sources take the most time
- [x] **PROF-03**: Profiler outputs actionable suggestions for slow components

### Neovim Plugin Audit

- [x] **NVIM-01**: Script analyzes Neovim plugin usage via :Lazy (or JSON output)
- [x] **NVIM-02**: Plugin audit identifies plugins loaded but not used (via standard indicators)
- [x] **NVIM-03**: Audit provides recommendations for plugins to remove

### Automated Tests

- [x] **TEST-01**: Test suite validates zsh syntax (`zsh -n`) for all zsh config files
- [x] **TEST-02**: Test suite validates Neovim config loads without errors (`nvim --headless`)
- [x] **TEST-03**: Test suite validates Git config syntax
- [x] **TEST-04**: Test suite runs via Makefile (`make test` or `make lint`)
